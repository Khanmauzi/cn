...
class Myudp : public ACE_Event_Handler{
	public :
	Myudp(ACE_INET_Addr &server)
	{
		data.open(server);
	}
	...
	int handle_input(ACE_HANDLE)
	{
		char buf[100],ans[100];
		ACE_INET_Addr cli;
		if(data.recv(buf,100,cli)<1) { return -1; }
		cout<<"client : "<<buf<<endl;
		int i; for(i=0;i<strlen(buf);i++) { ans[i]=toupper(buf[i]); }
		ans[i]='\0';
		data.send(ans,100,cli);
		return 0;
	}

	ACE_HANDLE get_handle()const{
		return this->data.get_handle();
	}
...
int main()
{
	ACE_INET_Addr s1("127.0.0.1:3000"),s2("127.0.0.1:4000"),s3("127.0.0.1:5000"),s4("127.0.0.1:6000");
	...
	Myudp *obj = new Myudp(s4);
	ACE_Reactor::instance()->register_handler(obj,ACE_Event_Handler::READ_MASK);
